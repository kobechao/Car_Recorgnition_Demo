...
def listCarInfo() :
	...
	if( form.get( 'IDNumber', None ) and form.get('carAddr', None) ) :
		if( form['carAddr'] in session['carAddrs'] ) :
			...
			personalCarcontract = eth.contract( address=form['carAddr'], abi=arg['ABI'] )
			carList = personalCarcontract.functions.getCarInfos().call( {'to': form['carAddr']})
			...
			# equipmentList = personalCarcontract.functions.getCarEquipment().call( {'to': form['carAddr']})
			# res['equipmentList'] = equipmentList
